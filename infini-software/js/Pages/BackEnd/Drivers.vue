    ...
    <div class="max-w-7xl px-4 mx-auto sm:px-6 lg:px-8 pt-4">
        ...
        <div class="flex justify-end me-4 mb-2">
            ...
            <input type="text"
                ...
                name="" v-model="search" placeholder="Rechercher un chauffeur.." id="" />
            ...
            <table class="w-full text-sm text-center text-gray-500  ">
                ...
                <tbody>
                    <tr class="cursor-pointer hover:bg-sky-100" :class="{ 'bg-slate-100': index % 2 == 1 }"
                        v-for="(driverData, index) in driversData.data" :key="driverData.id">

                        <td class="td py-2 flex justify-center"> <img :src="driverData.profile_image_url"
                                class="w-10 h-10 rounded-full object-cover" alt="" sizes="" srcset=""> </td>
                        <td class="td"> {{ driverData.firstName }} </td>
                        <td class="td">{{ driverData.lastName }} </td>
                        <td class="td">{{ driverData.phoneNumber }}</td>
                        <td class="td">
                            <span v-if="driverData.isPhoneVerified == 1" class="text-green-500">Active</span>
                            <span v-else class="text-red-500">Inactive</span>
                        </td>
                        <td class="td">
                            <!-- <i class="fa fa-edit text-yellow-500 me-3 cursor-pointer" @click="editProf(driverData)"></i> -->
                            <i class="fa fa-trash text-red-500 cursor-pointer px-4"></i>
                            <Link :href="route('chauffeur.details', driverData.firebase_uid)">Voir</Link>

                        </td>
                    </tr>
                </tbody>
            </table>
            <!-- <div v-if="driversData && driversData.data && driversData.data.length > 9" class="mt-2 flex justify-end">
          <Paginated :links="driversData.meta.links" />
        </div> -->
