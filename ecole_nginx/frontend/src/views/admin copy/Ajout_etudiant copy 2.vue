...
const toggleParcours = async (index, classes_id, niveauId, annee_academique, studentId) => {
  ...
  try {
    ...
    if (r.status === 200) parcours_cours.value = r.data.cours;
  } catch (e) { console.error(e); }
};
      ...
      <form @submit.prevent="submitEtudiant">
        <Transition name="tab-slide" mode="out-in">
          ...
          <div v-else-if="activeTab === 'details'" key="details" class="space-y-5">
            ...
            <!-- Parcours -->
            <div v-if="studentDetails && (studentDetails.classe_etudiant?.length || studentDetails.etudiant_facultes?.length)"
              ...
              <div class="divide-y divide-[#21262d]">
                <!-- Classique -->
                <template v-if="studentDetails.classe_etudiant">
                  <div v-for="(p, i) in studentDetails.classe_etudiant" :key="`cl-${i}`">
                    <button type="button"
                    ...
                    <Transition name="accordion">
                      <div v-if="openAccordionIndex === i" class="px-6 pb-4 space-y-4">
                        <div v-if="parcours_cours.length">
                          <p class="text-[11px] uppercase tracking-widest text-[#7d8590] mb-2">Cours suivis</p>
                          <div class="flex flex-wrap gap-2">
                            <span v-for="c in parcours_cours" :key="c.id"
                              class="text-[11px] bg-[#1f6feb]/10 border border-[#1f6feb]/20 text-[#58a6ff] px-2 py-1 rounded-md">
                              {{ c.cours_nom }}
                            </span>
                          </div>
                        </div>
                        <div class="grid grid-cols-3 gap-3">
                          <div v-for="blk in ['📊 Suivi pédagogique','💰 Suivi financier','🧾 Vie scolaire']" :key="blk"
                            class="bg-[#0d1117] rounded-lg p-3 border border-[#21262d]">
                            <span class="text-[11px] text-[#7d8590]">{{ blk }}</span>
                          </div>
                        </div>
                      </div>
                    </Transition>
