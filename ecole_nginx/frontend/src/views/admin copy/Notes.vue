...
// // --- ACTION D'IMPRESSION (SANS FORMULAIRE) ---
// const handlePrintBulletin = (studentId, evalType) => {
//   const query = new URLSearchParams({ bulletin: studentId });

//   // On injecte la valeur sélectionnée selon le type d'évaluation de l'élève
//   const type = evalType?.toLowerCase();
//   if (type === 'mois') query.append('mois', selections.value.mois);
//   else if (evalType === 'Trimestre') query.append('Trimestre', selections.value.Trimestre);
//   else if (evalType === 'Controle') query.append('Controle', selections.value.Controle);
//   else query.append('session', selections.value.session);

//   // Ouverture du PDF dans un nouvel onglet
//   window.open(`${url}/imprime-bulletin?${query.toString()}`, '_blank');
// };
...
<template>
  <div class="max-w-7xl px-4 mx-auto pb-16 text-slate-600">
      ...
      <router-link to="/admin/ajouter-notes" class="btn-outline-green">
        ...
        <table class="w-full text-sm text-center">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              ...
              <td class="px-4 py-3 ">{{ note.fname }}</td>
              ...
              <td class="px-4 py-3">
                ...
                <button 
                    ...
                    class="p-1 bg-white border border-slate-200 rounded shadow-sm hover:text-sky-600 hover:border-sky-200 transition-all cursor-pointer"
                  >
                    <i class="ri-file-pdf-2-line"></i>
                  </button>
                  <!-- <button 
                    @click="handlePrintBulletin(note.id, note.evaluation_par)"
                    class="p-2 bg-white border border-slate-200 rounded shadow-sm hover:text-sky-600 hover:border-sky-200 transition-all"
                  >
