...
<template>
  <div class="max-w-7xl px-4 mx-auto pt-8 pb-14">
    <div class="mb-6">
      <h1 class="text-2xl  text-slate-400">Ajouter des Matières</h1>
      <p class="text-slate-500">Vous pouvez ajouter plusieurs lignes pour enregistrer plusieurs cours à la fois.</p>
    </div>

    <form @submit.prevent="submitCours" class="bg-[#0d1117] p-6 rounded-xl shadow-sm border border-slate-400">
      
      <div v-for="(cours, index) in courses" :key="index" 
           class="grid md:grid-cols-3 gap-4 p-4 items-end transition-all bg-[#161b22]"
           :class="{ 'border-t border-slate-400 mt-2': index > 0 }">
      ...
      <div class="mt-8 pt-6 border-t border-slate-500 flex flex-col md:flex-row justify-between items-center gap-4">
        <button 
          type="button"
          @click="addCours" 
        ...
        <PrimaryButton 
          ...
          class="w-full md:w-auto px-10 py-3 bg-sky-800 hover:bg-sky-700 text-white rounded-lg shadow-md transition-all uppercase tracking-widest text-xs cursor-pointer"
        >
