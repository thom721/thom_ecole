...
<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-6 py-8 pb-20 text-slate-700 font-sans">

    <div class="flex items-center gap-3 mb-4">
      <div class="w-9 h-9 rounded-lg bg-sky-100 flex items-center justify-center text-sky-600 text-lg shrink-0">🏫</div>
      <div>
        <h1 class="text-lg font-bold text-slate-800 leading-tight">Profil de l'école</h1>
        <p class="text-xs text-slate-500">Informations générales et coordonnées</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h1 class="text-lg font-bold text-slate-800 leading-tight">Rôles &amp; Permissions</h1>
        <p class="text-xs text-slate-500">Gérer les accès par rôle et par utilisateur</p>
      </div>
    </div>

    <div v-if="hasPermission('Voir role')" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-8 space-y-8">
      ...
      <!-- ── PERMISSIONS ── -->
      <div v-if="hasPermission('Voir permission')" class="border-t border-slate-100 pt-6">
        <h2 class="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
        ...
        <form @submit.prevent="assignPermission">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
            ...
            <!-- Select rôle -->
            <div class="flex flex-col gap-1">
              <label class="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Rôle cible</label>
              <div class="relative">
                <select v-model="formPermission.role" @change="getPermissionByRole"
                  class="w-full appearance-none border border-slate-200 rounded-lg px-3 py-2 pr-8 text-sm text-slate-700 bg-slate-50 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100 transition">
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h1 class="text-lg font-bold text-slate-800 leading-tight">Mon profil</h1>
        <p class="text-xs text-slate-500">Vos informations personnelles</p>
      </div>
    </div>
