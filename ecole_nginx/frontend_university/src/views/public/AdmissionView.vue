<template>
  <div class="min-h-screen bg-[#0b0f14] text-[#dde4ed] font-sans">
    ...
    <!-- ═══════════════════════ PROCÉDURE (info) ══════════════════ -->
    <section class="max-w-[880px] mx-auto px-5 pt-8">
      <button type="button" @click="showProcedure = !showProcedure"
        class="w-full flex items-center justify-between gap-3 bg-[#111820] border border-[#252d38] rounded-2xl px-6 py-4 text-left hover:border-[#c9a84c]/40 transition-colors">
        <span class="flex items-center gap-3 min-w-0">
          <span class="text-xl flex-shrink-0">📋</span>
          <span class="min-w-0">
            <span class="block text-sm font-bold text-[#dde4ed]">Comprendre la procédure d'admission</span>
            <span class="block text-xs text-[#6b7a8d] mt-0.5">Ce qui se passe après votre demande en ligne — registrariat, paiement, examen, résultats.</span>
          </span>
        </span>
        <svg class="w-4 h-4 text-[#6b7a8d] transition-transform flex-shrink-0" :class="showProcedure ? 'rotate-180' : ''"
          fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 16 16">
          <polyline points="4 6 8 10 12 6"/>
        </svg>
      </button>

      <div v-if="showProcedure" class="card">
        <div class="flex gap-2 mb-6 overflow-x-auto pb-1">
          <button v-for="(p, key) in ADMISSION_PROCESS" :key="key" type="button"
            @click="procedureTab = key"
            class="flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-colors"
            :class="procedureTab === key
              ? 'bg-[#c9a84c] text-[#0b0f14]'
              : 'bg-[#181f28] border border-[#2e3845] text-[#6b7a8d] hover:text-[#dde4ed]'">
            {{ p.label }}
          </button>
        </div>

        <p v-if="currentProcedure.intro" class="text-sm text-[#9aa7b5] leading-relaxed mb-6">{{ currentProcedure.intro }}</p>

        <ol class="flex flex-col gap-5 list-none m-0 p-0">
          <li v-for="(e, i) in currentProcedure.etapes" :key="i" class="flex gap-4">
            <span class="flex-shrink-0 w-7 h-7 rounded-full bg-[#1f8f82]/15 border border-[#1f8f82]/40 text-[#3aad7a] text-xs font-bold flex items-center justify-center">{{ i + 1 }}</span>
            <div class="min-w-0">
              <h3 class="text-sm font-bold text-[#dde4ed] m-0 mb-1.5">{{ e.titre }}</h3>
              <p v-if="e.texte" class="text-xs text-[#9aa7b5] leading-relaxed m-0" :class="e.liste ? 'mb-2' : ''">{{ e.texte }}</p>
              <ul v-if="e.liste" class="list-disc pl-4 flex flex-col gap-1 m-0">
                <li v-for="(l, li) in e.liste" :key="li" class="text-xs text-[#9aa7b5] leading-relaxed">{{ l }}</li>
              </ul>
            </div>
          </li>
        </ol>

        <div v-if="currentProcedure.notes" class="mt-6 pt-5 border-t border-[#252d38] flex flex-col gap-2">
          <p v-for="(n, ni) in currentProcedure.notes" :key="ni" class="text-xs text-[#6b7a8d] leading-relaxed m-0">⚠ {{ n }}</p>
        </div>
      </div>
    </section>
        ...
        <Transition name="panel" mode="out-in">
          ...
          <div v-if="done" key="done"
            ...
            <p class="text-sm text-white/60 max-w-sm mx-auto leading-relaxed mb-4">
              Notre équipe examinera votre candidature sous 24 h. Une copie de votre fiche d'inscription vient de vous être envoyée par email.
            </p>
            <div class="inline-block bg-white/[.08] border border-white/[.12] text-white/75 text-sm px-5 py-2 rounded-full mb-6">
             ...
             <button  @click="submitPdf(`/public-recu-inscription/${student_id}`, { student_id: student_id }, 'print-recu-inscrit')" type="submit" class="btn-gold":disabled="loadingMap['print-recu-inscrit'] === true">
              ...
              <span v-else>📄 Télécharger ma fiche d'inscription</span>
            </button>
          ...
          <div v-else-if="step === 0" key="s0" class="card">
            ...
            <!-- Identité -->
            <Fieldset legend="Identité du candidat">
            ...
            <Fieldset legend="Formation souhaitée" class="mt-4">
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                ...
                <FormField label="Session / Année d'étude *" :error="f.errors.classe_actuelle_id">
          ...
          <div v-else-if="step === 3" key="s3" class="card">
            ...
            <!-- Bloc Candidat -->
            <RecapSection title="Candidat" icon="👤" @edit="step = 0">
            ...
            <RecapSection title="Formation" icon="📚" @edit="step = 0">
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3">
                ...
                <RecapField label="Session / Année d'étude" :val="classeLabel" />
...
/* ── Procédure d'admission (contenu informatif) ─────────── */
const ADMISSION_PROCESS = {
  premiere: {
    label: '1ère année',
    intro: "Vous souhaitez poursuivre vos études après le BAC et intégrer l'IUSTH ? Voici les étapes à suivre. Vous pouvez passer les tests d'admission sous réserve de valider votre Baccalauréat.",
    etapes: [
      { titre: "Étape 1 — Demande d'admission en ligne", liste: ["Faire une demande d'admission"] },
      { titre: "Étape 2 — Dépôt du dossier au registrariat", texte: "Le postulant devra se rendre au registrariat et soumettre toutes les pièces suivantes :", liste: [
        "Formulaire de demande d'admission (rempli en ligne puis imprimé)",
        "Une expédition de l'acte de naissance (original et une copie)",
        "Le dossier scolaire complet : certificat de fin d'études secondaires classiques (Bac II ou équivalent) et relevés de notes du baccalauréat II",
        "Deux photos d'identité récentes (maximum 3 mois)",
      ] },
      { titre: "Étape 3 — Paiement des frais d'admission", texte: "Après validation du registrariat, le postulant se rend à la Comptabilité de l'Université, muni de son dossier complet, pour payer les frais d'admission (non-remboursables). Un reçu de paiement lui est délivré — à conserver, il sera demandé le jour de l'examen." },
      { titre: "Étape 4 — Fiche d'accès à l'examen (Décanat)", texte: "Le postulant se rend au décanat de la Faculté choisie, muni de son dossier complet et du reçu de paiement. Il reçoit sa fiche d'accès à l'examen (code postulant), obligatoire pour accéder aux salles d'examen." },
      { titre: "Étape 5 — Examen d'admission", texte: "Le postulant passe obligatoirement les tests d'évaluation lors de l'examen d'admission. Les matières et leur contenu sont définis par chaque faculté selon ses programmes." },
      { titre: "Étape 6 — Résultats et intégration", texte: "Les résultats sont affichés devant les Décanats. En cas de réussite, l'étudiant est admis soit comme étudiant régulier (choix de cours en ligne, réunion d'accueil par Faculté), soit comme étudiant libre (cheminement spécial de 2 sessions maximum, intégration possible avec 60/100 de moyenne générale ou en repassant l'examen après 1 session)." },
    ],
    notes: [
      "Étudiant libre — cas d'échec : après 2 sessions sans intégrer une faculté (ni par la moyenne, ni par l'examen), l'étudiant doit refaire le processus d'admission mais ne peut plus se réinscrire comme étudiant libre.",
    ],
  },
  readmission: {
    label: 'Ré-admission',
    intro: "En cas d'absence, le dossier de l'étudiant peut être rouvert selon les conditions suivantes.",
    etapes: [
      { titre: "1 session d'absence", texte: "L'étudiant peut rouvrir son dossier sans frais." },
      { titre: "2 sessions d'absence ou plus", texte: "L'étudiant doit payer des frais de réouverture de dossier." },
    ],
  },
  dossier: {
    label: 'Sur dossier (transfert)',
    intro: "Le transfert d'une autre université est traité sur dossier.",
    etapes: [
      { titre: "Étape 1 — Demande d'admission en ligne", liste: ["Faire une demande d'admission en ligne"] },
      { titre: "Étape 2 — Constitution du dossier", texte: "L'étudiant doit présenter les pièces suivantes :", liste: [
        "Formulaire de demande d'admission (rempli en ligne puis imprimé)",
        "Une expédition de l'acte de naissance (original et une copie)",
        "Le dossier scolaire complet : certificat de fin d'études secondaires (Bac II ou équivalent), relevés de notes du baccalauréat II et relevés de notes universitaires",
        "Deux photos d'identité récentes (maximum 3 mois)",
        "Pour les postulants en Cycle court : une copie de la lettre de travail",
      ] },
      { titre: "Étape 3 — Paiement des frais", texte: "Après validation, le postulant se rend à la Comptabilité, muni de son dossier complet, pour payer les frais d'admission (non-remboursables). Un reçu de paiement lui est délivré, à conserver comme preuve." },
      { titre: "Étape 4 — Dépôt au décanat", texte: "Le postulant se rend au décanat de la Faculté choisie, muni de son dossier complet (pièces de l'étape 2 et reçu de paiement)." },
      { titre: "Étape 5 — Décision", texte: "L'étudiant est contacté par le Décanat de la Faculté à laquelle il a postulé, qui lui communique la décision." },
      { titre: "Étape 6 — Intégration", texte: "L'étudiant accepté procède ensuite à son choix de cours en ligne. Les cheminements types sont accessibles en ligne." },
    ],
  },
}
const showProcedure = ref(false)
const procedureTab  = ref('premiere')
const currentProcedure = computed(() => ADMISSION_PROCESS[procedureTab.value])
