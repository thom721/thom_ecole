  ...
  <div class="min-h-screen bg-[#0d0d14] flex overflow-hidden animate-[fadeUp_0.4s_ease_both]">
    ...
    <div class="hidden lg:flex flex-col justify-between w-[52%] relative overflow-hidden p-12"
      ...
      <div class="relative z-10">
        <div class="flex items-center gap-3">
          <div class="w-24 h-24 rounded-full flex items-center justify-center text-xl"
            :style="{ background: 'rgba(255,255,255,0.15)' }">
            <img v-if="useSchoolInfo.school_info?.logo_image_base64" :src="useSchoolInfo.school_info?.logo_image_base64" :alt="useSchoolInfo.school_info?.nom || 'Logo'" class="object-contain flex items-center justify-center w-16 h-16">
          </div>
          <span class="font-serif text-white text-xl font-bold tracking-wide">{{useSchoolInfo.school_info?.nom || 'IUSTH'}}</span>
        </div>
      </div>
    ...
    <div class="flex-1 flex flex-col justify-center px-8 sm:px-12 lg:px-16 bg-[#0d0d14] relative">
      ...
      <div class="lg:hidden flex items-center gap-2 mt-4">
         <img v-if="useSchoolInfo.school_info?.logo_image_base64" :src="useSchoolInfo.school_info?.logo_image_base64" :alt="useSchoolInfo.school_info?.nom || 'Logo'" class="object-contain flex items-center justify-center w-12 h-12">
        <span v-else class="text-2xl">🎓</span>
        <span class="font-serif text-[#c9a84c] text-xl font-bold">{{useSchoolInfo.school_info?.nom || 'IUSTH'}}</span>
      </div>

      <div class="max-w-md w-full mx-auto">
        <p class="text-start text-xs text-gray-500 mt-6 mb-2">
          ...
          <div class="flex gap-2">
            ...
            <button
              ...
              :class="selectedRole === role.key
                ? 'border-[#c9a84c] text-white bg-white/5'
                : 'border-[#222] text-[#555] hover:border-[#333] hover:text-[#888]'"
              class="flex-1 flex flex-col items-center gap-1.5 py-3 px-2 rounded-lg border transition-all cursor-pointer">
              <span class="text-xl">{{ role.emoji }}</span>
              <span class="text-xs font-semibold">{{ role.label }}</span>
            </button>
          </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input
                v-model="form.email"
                :type="selectedRole === 'etudiant' ? 'text' : 'email'"
                :placeholder="activeRole.emailPlaceholder"
                class="w-full bg-[#111] border border-[#222] rounded-lg pl-11 pr-4 py-3 text-white text-sm placeholder-[#333] outline-none transition-all focus:border-[#c9a84c] focus:ring-1 focus:ring-[#c9a84c]/30"
          ...
          <button type="submit"
            ...
            class="w-full py-3 rounded-lg font-semibold text-sm transition-all cursor-pointer mt-2 relative overflow-hidden"
        ...
        <p class="text-center text-[#333] text-xs mt-10">
          © {{ new Date().getFullYear() }} {{ useSchoolInfo.school_info?.nom || 'IUSTH' }} · Tous droits réservés
        </p>
      </div>
    </div>
