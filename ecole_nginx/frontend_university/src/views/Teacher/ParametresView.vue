      ...
      <div class="bg-[#171b26] border border-white/[0.07] rounded-2xl p-6 flex flex-col gap-5">
        ...
        <div class="flex items-center gap-4 pb-5 border-b border-white/[0.07]">
          ...
          <div>
            ...
            <p class="text-[12px] text-[#7c83a0]">Professeur(e) {{professerDetails?.matiere_enseignee}}</p>
          </div>
        </div>
        <div class="flex flex-col gap-3">
          <div v-for="f in profileFields" :key="f.label" class="flex flex-col gap-1.5">
            ...
            <p class="bg-[#1e2335] border border-white/10 rounded-md px-4 py-2.5 text-[#e8eaf0] text-sm w-full min-h-[2.5rem] flex items-center">{{ f.value || '—' }}</p>
          </div>
        </div>
      </div>
...
const profileFields = computed(() => [
  { label:'Prénom',              value: professerDetails.value?.prenom },
  { label:'Nom',                 value: professerDetails.value?.nom },
  { label:'Nom d\'utilisateur',  value: professerDetails.value?.user?.username },
  { label:'Email',               value: professerDetails.value?.user?.email },
  { label:'Matière principale',  value: professerDetails.value?.matiere_enseignee },
])
...
onMounted( async () => {
  const { data } = await axios.get(`/mon-professeur`)
