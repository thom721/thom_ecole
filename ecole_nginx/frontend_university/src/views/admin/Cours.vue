...
const deleteCours = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        // Bug corrigé : il n'existe aucune route DELETE /cours/{id} côté
        // serveur — la vraie route est GET /delete-cours/{id} (RCours.py).
        await axios.get(`${url}/delete-cours/${id}`);
        searchCours();
      } catch (e) {
        Swal.fire('Erreur', e.response?.data?.detail || 'Impossible de supprimer ce cours.', 'error');
      }
    }
  });
};
...
const deleteProgramme = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        await axios.get(`${url}/delete-programme/${id}`);
        searchProgramme();
      } catch (e) {
        Swal.fire('Erreur', e.response?.data?.detail || 'Impossible de supprimer ce programme.', 'error');
      }
    }
  });
};
  ...
  >
    ...
    <Transition name="tab-slide" mode="out-in">
              ...
              >
                ...
                <td class="px-4 py-2.5">
                  <div class="flex items-center justify-end gap-1">
                    <router-link
                      :to="`/admin/ajouter-cours?id=${cours.id}`"
      ...
      <div v-else-if="activeTab === 'programmes'" key="programmes">
        ...
        <div class="w-full">
            <InputLabel value="Session / Année d'étude" />
              <select v-model="programme_class" class="field-input">
               <option value="" disabled>Choisir Session / Année d'étude</option>
          ...
          <table class="w-full border-collapse">
            <thead>
              <tr class="bg-[#0d1117] border-b border-white/[0.05]">
                ...
                <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.07em] text-[#3d4d62]">Session / Année d'étude</th>
                <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold uppercase tracking-[0.07em] text-[#3d4d62]">Année</th>
                <th class="px-4 py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-[0.07em] text-[#3d4d62]">Actions</th>
              </tr>
            </thead>
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-2.5">
                  <div class="flex items-center justify-end gap-1">
                    <router-link
                      :to="`/admin/ajouter-programme?id=${prog.id}`"
                      class="w-7 h-7 rounded-lg flex items-center justify-center text-[#3d4d62]
                             border border-transparent
                             hover:bg-[#4a7cff]/10 hover:border-[#4a7cff]/20 hover:text-[#7aaeff]
                             transition-all duration-150"
              ...
              <tr v-if="!dataProgramme.data?.length">
                <td colspan="6" class="py-14 text-center text-[#3d4d62] text-[13px]">
      ...
      <template #content>
        <form id="horaire-form" @submit.prevent="handlePrintHoraire" class="flex flex-col gap-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="flex flex-col gap-1.5">
              <label class="text-[11px] font-medium uppercase tracking-[0.06em] text-[#3d4d62]">Session / Année d'étude</label>
              <div class="relative">
                <select
