...
// (configurable dans Profil > Vues > Étudiant, voir adProfile.vue ALL_NAV) —
// canSeeSubTab() ne fait que RESTREINDRE davantage un bouton déjà autorisé
// par canWrite quand ce dernier s'applique, jamais accorder un droit
// d'écriture que le rôle n'a pas. Même logique que côté Flutter
// (etudiant_screen.dart::subOk).
...
const deleteStudent = async (id) => {
  if (!confirm("Voulez-vous vraiment supprimer cet étudiant ? Cette action est irréversible.")) return;
  try {
    await axios.delete(`${url}/etudiant/${id}`);
    showSwalInfo("Étudiant supprimé.", "green");
    searchEtudiants();
  } catch (error) {
    showSwalInfo(error.response?.data?.detail || "Erreur lors de la suppression.", "red");
  }
};
...
const actions = computed(() => {
  const base = [];
  // Restrictions Vues ajoutées SANS changer les restrictions de rôle déjà
  // en place (edit/view n'ont jamais exigé canWrite ici, contrairement à
  // Flutter — pas dans le périmètre de ce changement) : canSeeSubTab()
  // vient uniquement RESTREINDRE davantage un bouton déjà autorisé.
  if (canSeeSubTab('etudiant.modifier')) {
    base.push({
      key: 'edit',
      type: 'slot',
  ...
  if (canWrite.value && canSeeSubTab('etudiant.supprimer')) {
    base.push({
      key: 'delete',
      type: 'button',
      icon: 'ri-delete-bin-6-line text-rose-400 hover:bg-rose-600/10 text-md',
      onClick: async (row) => { deleteStudent(row.id); },
    });
  }
  return base;
});
  ...
  <div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 pb-4">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <router-link v-if="canWrite && canSeeSubTab('etudiant.ajouter')" to="/admin/etudiants/ajouter"
        ...
        <button v-if="canWrite && canSeeSubTab('etudiant.importer')" type="button"
          class="btn-emerald">
        ...
        <!-- Un seul bouton web pour Diplôme + Certificat (Flutter les
        sépare en deux vues distinctes) — visible si l'une OU l'autre est
        autorisée, puisqu'il donne accès aux deux à la fois. -->
        <button v-if="canSeeSubTab('etudiant.diplome') || canSeeSubTab('etudiant.certificat')"
          @click="searchForCertificate = true"
          class="btn-violet">
          <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" class="w-3.5 h-3.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5"/>
          </svg>
