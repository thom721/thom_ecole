<template>
  <div class="text-slate-500 min-h-screen  animate-[fadeUp_0.4s_ease_both]">
    <div class="max-w-7xl mx-auto p-2 sm:px-6 lg:px-8 gap-8">
      
      <div class="flex flex-col md:flex-row w-full gap-8 mt-4">
        <div class="p-4 bg-[#161b22] rounded-lg border w-full shadow-sm">
          <h2 class="text-xl  text-center border-b pb-2 mb-4">📊 Rapport Global</h2> 
          <div class="flex items-center gap-12 mb-4  bg-[#161b22]">
            <p class="text-xl ">- Rapport</p>
            <div class="flex-1">
              <label class="block text-sm">Type</label>
              <select v-model="forms.global.type" class="field-select">
            ...
            <div class="w-full mt-2">
              ...
              <input type="date" v-model="forms.global.date_debut" class="field-input" />
            </div>

            <div class="w-full mt-2">
              <label class="block text-sm">Fin</label>
              <input type="date" v-model="forms.global.date_fin" class="field-input" />
            </div>
          <!-- </div> -->
       ...
       <div class="w-full p-4 border rounded shadow-sm bg-[#161b22]">
          ...
          <div class="px-4 space-y-3">
            <div>
              ...
              <select v-model="forms.payment.classe" class="field-select">
            ...
            <label class="text-sm">Année Académique</label>
             <select v-model="forms.payment.date_debut" class="field-select">
            ...
            <div>
              ...
              <select v-model="forms.payment.versement" class="field-select">
        ...
        <div class="w-full p-4 border rounded shadow-sm bg-[#161b22]">
          ...
          <div class="space-y-3">
            <div class="flex gap-2">
              <select v-model="forms.pedago.cycle" class="field-select">
                <option value="All">Tous les Cycles</option>
                <option v-for="n in niveau" :key="n.id" :value="n.id">{{ n.name }}</option>
              </select>
              <select v-model="forms.pedago.classe" class="field-select">
            ...
            <div class="flex jutify-between items-center">
                  <select v-model="forms.pedago.annee_ac" class="field-select">
                    <option value="" disabled>Choisir une Année</option>
                    <option v-for="a in annee" :key="a.id" :value="a.id">{{ a.annee_academique }}</option>
                  </select>
              <select v-model="forms.pedago.mois" class="field-select">
      ...
      <div class="flex flex-col md:flex-row justify-between gap-8 mt-4">        
        <div class="w-full p-4 border rounded shadow-sm bg-[#161b22]">
          ...
          <div class="space-y-3 mb-6   p-2 rounded">
             <div class="flex gap-2">
               <select v-model="forms.admin.cycle" class="field-select text-xs">
                 <option value="All">Tous Cycles</option>
                 <option v-for="n in niveau" :key="n.id" :value="n.id">{{ n.name }}</option>
               </select>
               <select v-model="forms.admin.classe" class="field-select text-xs">
               ...
               {{ cls.nom_classe }}
                  ...
                  <div> 
            <label class="text-sm">Année Académique</label>
             <select v-model="forms.admin.annee_ac" class="field-select">
              <option value="" disabled>Choisir une Année</option>
              <option v-for="a in annee" :key="a.id" :value="a.id">{{ a.annee_academique }}</option>
            </select>
            </div>
          ...
          <div class="flex flex-col gap-2 bg-[#161b22] p-2 rounded">
            <div class="flex gap-2">
              <input type="date" v-model="forms.presence.date_debut" class="field-select text-xs" />
              <input type="date" v-model="forms.presence.date_fin" class="field-select text-xs" />
            </div>
