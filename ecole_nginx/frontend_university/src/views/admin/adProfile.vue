...
// Horaires par défaut (repris de l'ancien affichage codé en dur de
// ContactView.vue) — utilisés tant que l'admin n'a jamais encore enregistré
// d'horaires, pour ne pas afficher un bloc vide sur la page publique.
...
const profileForm = ref({
  ...
  whatsapp_url: '', facebook_url: '', tiktok_url: '', youtube_url: '',
  horaires: DEFAULT_HORAIRES.map(h => ({ ...h })),
  is_receive_arriere: false, inscription: false,
})

const addHoraireRow = () => { profileForm.value.horaires.push({ jour: '', horaire: '' }) }
const removeHoraireRow = (i) => { profileForm.value.horaires.splice(i, 1) }
...
const getProfile = async () => {
  try {
    ...
    Object.assign(profileForm.value, {
      ...
      whatsapp_url: d.whatsapp_url ?? '', facebook_url: d.facebook_url ?? '',
      tiktok_url: d.tiktok_url ?? '', youtube_url: d.youtube_url ?? '',
      horaires: (d.horaires && d.horaires.length) ? d.horaires.map(h => ({ ...h })) : DEFAULT_HORAIRES.map(h => ({ ...h })),
      is_receive_arriere: d.is_receive_arriere ?? false,
      inscription: d.inscription ?? false,
    })
...
const submitProfile = async () => {
  ...
  try {
    ...
    else notify(err.response?.data?.detail || 'Erreur lors de la sauvegarde.', false)
  } finally {
      ...
      <div v-if="!authStore.isBaseUser && canSeeSection('profile.ecole')" class="bg-[#161b26] rounded-2xl border border-white/[0.07] p-6 mb-6">
        <form @submit.prevent="submitProfile">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2 flex items-start gap-3 p-3 rounded-xl bg-[#0d1117] border border-white/[0.06]">
              <button type="button"
                @click="profileForm.inscription = !profileForm.inscription"
                class="relative w-9 h-5 rounded-full transition-colors shrink-0 mt-0.5"
                :class="profileForm.inscription ? 'bg-[#4f8ef7]' : 'bg-white/[0.08]'">
                <span class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all"
                  :class="profileForm.inscription ? 'left-[18px]' : 'left-0.5'"></span>
              </button>
              <div>
                <p class="text-[12px] font-semibold text-[#c9d1d9]">Inscriptions ouvertes</p>
                <p class="text-[11px] text-[#7c83a0] mt-0.5">Affiche le bloc "Les dossiers de candidature sont ouverts" sur la page d'accueil publique.</p>
              </div>
            </div>
          ...
          <div class="mt-6 pt-5 border-t border-white/[0.05]">
            <p class="text-[12px] font-bold text-[#c9d1d9] mb-3">Réseaux sociaux</p>
            <p class="text-[11px] text-[#7c83a0] mb-3">Un lien laissé vide n'apparaît pas du tout sur la page Contact publique (pas de lien mort).</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5">
                <label class="field-label">💬 WhatsApp</label>
                <input v-model="profileForm.whatsapp_url" type="url" placeholder="https://wa.me/50900000000" class="field-input" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="field-label">📘 Facebook</label>
                <input v-model="profileForm.facebook_url" type="url" placeholder="https://facebook.com/..." class="field-input" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="field-label">🎵 TikTok</label>
                <input v-model="profileForm.tiktok_url" type="url" placeholder="https://tiktok.com/@..." class="field-input" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="field-label">▶️ YouTube</label>
                <input v-model="profileForm.youtube_url" type="url" placeholder="https://youtube.com/@..." class="field-input" />
              </div>
            </div>
          </div>
          ...
          <div class="mt-6 pt-5 border-t border-white/[0.05]">
            <div class="flex items-center justify-between mb-3">
              <p class="text-[12px] font-bold text-[#c9d1d9]">Horaires d'accueil</p>
              <button type="button" @click="addHoraireRow" class="text-[11px] font-semibold text-[#7aaeff] hover:text-[#4f8ef7]">+ Ajouter une ligne</button>
            </div>
            <div v-for="(h, i) in profileForm.horaires" :key="i" class="flex items-center gap-2 mb-2">
              <input v-model="h.jour" type="text" placeholder="Ex : Lundi – Jeudi" class="field-input flex-1" />
              <input v-model="h.horaire" type="text" placeholder="Ex : 8h00 – 17h00 (ou Fermé)" class="field-input flex-1" />
              <button type="button" @click="removeHoraireRow(i)" class="w-8 h-8 shrink-0 rounded-lg bg-white/[0.05] hover:bg-red-500/15 text-[#7c83a0] hover:text-red-400 flex items-center justify-center transition-colors">✕</button>
            </div>
            <p v-if="!profileForm.horaires.length" class="text-[11px] text-[#7c83a0]">Aucune ligne — le bloc horaires n'apparaîtra pas sur la page publique.</p>
          </div>
