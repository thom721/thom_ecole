...
const parcours_paiement       = ref(null); // notes — objet {moyenne_gen, top, low}
...
const submitEtudiant = async () => {
  ...
  formEtudiant.value.processing=true
  try {
  ...
  if (e.response?.data?.errors) {
    formEtudiant.value.errors = e.response.data.errors;
  }
   
    // if (e.response?.data?.detail) showSwalInfo(e.response.data.detail, 'red');
  }finally{
    formEtudiant.value.processing=false
  }
};
...
const fetchStudentDetails = async (studentId) => {
  try {
    ...
    const dC = data.classes_etudiant
      ?.slice()
    ...
    // Auto-déployer l'année la plus récente
    const sorted = (data.classes_etudiant || [])
      .slice()
      .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
    if (sorted.length) {
      const latestIndex = (data.classes_etudiant || []).findIndex(p => p.annee_academique_id === sorted[0].annee_academique_id)
      if (latestIndex >= 0) {
        await toggleParcours(latestIndex, sorted[0].classes_id, sorted[0].niveau_id, sorted[0].annee_academique_id, data.id)
      }
    }
  } catch (e) { console.error(e); }
};
...
const toggleParcours = async (index, classes_id, niveauId, annee_academique, studentId) => {
  ...
  // Reset
  parcours_cours.value    = []
  parcours_paiement.value = null
  parcours_notes.value    = []
  try {
    const r = await axios.post(`${url}/student-specific-details`, {
      student_id: studentId, classe_id: classes_id, annee_id: annee_academique, niveau_id: niveauId
    });
    if (r.status === 200) {
      // ── Notes & moyennes depuis parcours[0].details_bruts ──────────────
      const parcoursData = r.data.parcours ?? []
      if (parcoursData.length) {
        const details = parcoursData[0].details_bruts ?? {}
        const notesList = []
        for (const [, matieres] of Object.entries(details)) {
          for (const [matiere, mData] of Object.entries(matieres)) {
            const vals = Object.values(mData.notes ?? {}).map(Number).filter(v => !isNaN(v))
            const moy = vals.length ? (vals.reduce((a,b) => a+b, 0) / vals.length) : 0
            notesList.push({ cours_nom: matiere, note: moy.toFixed(2), coef: mData.coefficients ?? '' })
          }
        }
        parcours_paiement.value = parcoursData[0]  // Stocke l'objet entier pour la moyenne

        // Noms des cours pour "Cours suivis"
        parcours_cours.value = notesList.map(n => ({ cours_nom: n.cours_nom }))
      }

      // ── Suivi financier depuis paiement_details.info_paiement ─────────
      const pd = r.data.paiement_details ?? {}
      const info = pd?.paiement_details?.info_paiement ?? pd?.info_paiement ?? {}
      const versements = Object.entries(info)
        .filter(([, d]) => d?.status !== 'retourné')
        .map(([dateKey, d]) => {
          const sp = Array.isArray(d.status_paiement) ? d.status_paiement : []
          const acqt = sp.filter(s => s.startsWith('Acqt:')).map(s => s.replace('Acqt: ','')).join(', ')
          const avns = sp.filter(s => s.startsWith('Avns:')).map(s => s.replace('Avns: ','')).join(', ')
          return {
            date: dateKey,
            montant: d.depot ?? 0,
            total_verse: d.total_verse ?? 0,
            balance: d.balance ?? 0,
            statut: d.status === 'Acquitté' ? 'Acquitté'
                  : acqt ? `Acqt: ${acqt}`
                  : avns ? `Avance: ${avns}`
                  : 'En cours',
            description: acqt || avns || d.status || '—',
          }
        })
        .sort((a, b) => a.date.localeCompare(b.date))
      parcours_notes.value = versements   // Suivi financier
    }
  } catch (e) { console.error(e); }
};
...
<template>
  <div class="min-h-screen bg-[#0d1117] text-[#c9d1d9]  animate-[fadeUp_0.4s_ease_both]" style="font-family:'DM Sans','Segoe UI',sans-serif">
      ...
      <div class="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        <div class="flex flex-col md:flex-row justify-between items-center gap-3">
          ...
          <div class="flex flex-row gap-4">
             <button  @click="submitPdf(`/print-recu-inscrit/${formEtudiant.id}`, { student_id: formEtudiant.id }, 'print-recu-inscrit')" type="submit" class="action-btn action-btn--violet":disabled="loadingMap['print-recu-inscrit'] === true">              
            ...
            <button @click="submitPdf('/student-print-details', { student_id: formEtudiant.id }, formEtudiant.id)" type="submit" class="action-btn action-btn--emerald" :disabled="loadingMap[formEtudiant.id] === true">
              
              <svg v-if="loadingMap[formEtudiant.id] === true" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>Génération...
                </svg>
          ...
          <div v-if="activeTab === 'inscription'" key="inscription" class="space-y-5">
            ...
            <SectionCard title="Parcours scolaire" icon="📚">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <FormField label="Dernier établissement" :error="formEtudiant.errors?.dernier_etablissement">
                  <input class="field-input" type="text" v-model="formEtudiant.dernier_etablissement" placeholder="Nom de l'établissement" />
                </FormField>
                <FormField label="NISU" :error="formEtudiant.errors?.nisu">
            ...
            <SectionCard title="Identité" icon="🪪">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <FormField label="Date de naissance" :error="formEtudiant.errors?.date_de_naissance">
                  <input class="field-input" type="date" v-model="formEtudiant.date_de_naissance" />
                </FormField>
                <FormField label="Lieu de naissance" :error="formEtudiant.errors?.lieu_de_naissance">
                  <input class="field-input" type="text" v-model="formEtudiant.lieu_de_naissance" placeholder="Ville / Commune" />
                </FormField>
                <FormField label="Religion" :error="formEtudiant.errors?.religion">
            ...
            <SectionCard title="Contact" icon="📡">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <FormField label="Adresse" :error="formEtudiant.errors?.adresse">
                  <input class="field-input" type="text" v-model="formEtudiant.adresse" placeholder="Adresse complète" />
                </FormField>
                <FormField label="Téléphone" :error="formEtudiant.errors?.telephone">
                  <input class="field-input" type="text" v-model="formEtudiant.telephone" placeholder="+509 ..." />
                </FormField>
                <FormField label="Courriel" :error="formEtudiant.errors?.email">
            ...
            <SectionCard title="Inscription académique" icon="📋">
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <FormField label="Année académique" :error="formEtudiant.errors?.annee_academique_id">
                ...
                <FormField
                  ...
                  label="Faculté / Domaine" :error="formEtudiant.errors?.faculte_id"
                >
                ...
                <FormField label="Session / Année d'étude" :error="formEtudiant.errors?.classe_actuelle_id">
          ...
          <div v-else-if="activeTab === 'documents'" key="documents" class="space-y-4">
            ...
            <div class="flex justify-end">
              <!-- <button type="submit"
               ...
               <button type="submit" class="action-btn action-btn--blue" :disabled="formEtudiant.processing">
                <svg v-if="!formEtudiant.processing" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
                  <path d="M12.78 5.22a.749.749 0 0 1 0 1.06l-4.25 4.25a.749.749 0 0 1-1.06 0L5.22 8.28a.749.749 0 1 1 1.06-1.06L8 8.94l3.72-3.72a.749.749 0 0 1 1.06 0Z" />
                </svg>
              <svg v-if="formEtudiant.processing" class="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
              <span v-if="formEtudiant.id">{{ formEtudiant.processing ? 'Modifications en cours…' : 'Sauvegarder les modifications' }}</span>
               <span v-else>{{ formEtudiant.processing ? 'Enregistrement cours…' : 'Enregistrer l\'étudiant' }}</span>
            </button>
            </div>
          </div>
            ...
            <div v-if="studentDetails" class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div class="bg-[#161b22] border border-[#21262d] rounded-xl overflow-hidden">
                ...
                <div class="p-4 space-y-0.5">
                  ...
                  <InfoRow label="Session / Année d'étude actuelle" :value="classe_actuelle_"                     :accent="true" />
                </div>
              </div>
                    ...
                    <Transition name="accordion">
                      <div v-if="openAccordionIndex === i" class="border-t border-[#21262d]">
                        ...
                        <div class="px-5 py-3 flex items-center justify-between bg-[#0d1117]/60 border-b border-[#21262d]">
                          <span class="text-[12px] text-[#7d8590]">Session / Année d'étude fréquentée</span>
                        ...
                        <div class="p-5 space-y-5">
                          ...
                          <div>
                            ...
                            <div v-if="parcours_cours && parcours_cours.length" class="flex flex-wrap gap-2">
                          ...
                          <!-- 📊 Notes et moyennes — Moyenne générale -->
                          <div>
                            <p class="text-[11px] uppercase tracking-widest text-[#7d8590] mb-2 flex items-center gap-1">
                              <span>📊</span> Notes et moyennes
                            </p>
                            <div v-if="parcours_paiement?.moyenne_gen !== undefined"
                              class="bg-[#0d1117] border border-[#21262d] rounded-xl p-4 flex flex-col gap-3">

                              <!-- Moyenne générale -->
                              <div class="flex items-center justify-between">
                                <span class="text-[12px] text-[#7d8590]">Moyenne générale</span>
                                <div class="flex items-center gap-2">
                                  <span :class="['text-[26px] font-black font-mono',
                                    parcours_paiement.moyenne_gen >= 9 ? 'text-[#3fb950]' :
                                    parcours_paiement.moyenne_gen >= 7 ? 'text-[#d29922]' :
                                    parcours_paiement.moyenne_gen >= 6 ? 'text-[#58a6ff]' : 'text-[#f85149]']">
                                    {{ parcours_paiement.moyenne_gen }}<span class="text-[14px] font-normal text-[#7d8590]">/10</span>
                                  </span>
                                  <span :class="['text-[11px] px-2.5 py-1 rounded-full font-semibold',
                                    parcours_paiement.moyenne_gen >= 9 ? 'bg-[#1a4731] text-[#3fb950]' :
                                    parcours_paiement.moyenne_gen >= 7 ? 'bg-[#2d2a00] text-[#d29922]' :
                                    parcours_paiement.moyenne_gen >= 6 ? 'bg-[#1d2a3a] text-[#58a6ff]' : 'bg-[#3d1a1a] text-[#f85149]']">
                                    {{ parcours_paiement.moyenne_gen >= 9 ? 'Très Bien' :
                                       parcours_paiement.moyenne_gen >= 7 ? 'Bien' :
                                       parcours_paiement.moyenne_gen >= 6 ? 'Passable' : 'Échec' }}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div class="w-full h-2 bg-white/[0.06] rounded-full overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-700"
                                  :style="{
                                    width: (Math.min(parcours_paiement.moyenne_gen, 10) / 10 * 100) + '%',
                                    background: parcours_paiement.moyenne_gen >= 9 ? '#3fb950' :
                                                parcours_paiement.moyenne_gen >= 7 ? '#d29922' :
                                                parcours_paiement.moyenne_gen >= 6 ? '#58a6ff' : '#f85149'
                                  }"></div>
                              </div>

                              <!-- Top & Low matière -->
                              <div class="grid grid-cols-2 gap-3 pt-1">
                                <div v-if="parcours_paiement.top" class="bg-[#1a4731]/30 border border-[#2ea043]/20 rounded-lg px-3 py-2">
                                  <p class="text-[10px] text-[#7d8590] mb-0.5">🏆 Meilleure matière</p>
                                  <p class="text-[12px] text-[#3fb950] font-semibold">{{ parcours_paiement.top.nom }}</p>
                                  <p class="text-[11px] font-mono text-[#3fb950]">{{ parcours_paiement.top.moy }}/10</p>
                                </div>
                                <div v-if="parcours_paiement.low" class="bg-[#3d1a1a]/30 border border-[#f85149]/20 rounded-lg px-3 py-2">
                                  <p class="text-[10px] text-[#7d8590] mb-0.5">⚠ À améliorer</p>
                                  <p class="text-[12px] text-[#f85149] font-semibold">{{ parcours_paiement.low.nom }}</p>
                                  <p class="text-[11px] font-mono text-[#f85149]">{{ parcours_paiement.low.moy }}/10</p>
                                </div>
                              </div>
                            </div>
                              ...
                              <table class="w-full text-[12px]">
                                <thead>
                                  <tr class="border-b border-[#21262d]">
                                    <th class="text-left text-[10px] uppercase text-[#7d8590] px-3 py-2">Date versement</th>
                                    <th class="text-center text-[10px] uppercase text-[#7d8590] px-3 py-2">Dépôt</th>
                                    <th class="text-center text-[10px] uppercase text-[#7d8590] px-3 py-2">Total versé</th>
                                    <th class="text-center text-[10px] uppercase text-[#7d8590] px-3 py-2">Balance</th>
                                    <th class="text-center text-[10px] uppercase text-[#7d8590] px-3 py-2">Situation</th>
                                  </tr>
                                </thead>
                                <tbody class="divide-y divide-[#21262d]">
                                  <tr v-for="v in parcours_notes" :key="v.date" class="hover:bg-[#161b22] transition-colors">
                                    <td class="px-3 py-2 font-mono text-[#7d8590] text-[11px]">{{ v.date }}</td>
                                    <td class="px-3 py-2 text-center font-mono text-[#e6edf3]">{{ Number(v.montant).toLocaleString('fr-FR') }}</td>
                                    <td class="px-3 py-2 text-center font-mono text-[#3fb950] font-semibold">{{ Number(v.total_verse).toLocaleString('fr-FR') }}</td>
                                    <td class="px-3 py-2 text-center font-mono" :class="v.balance > 0 ? 'text-[#f85149]' : 'text-[#3fb950]'">
                                      {{ Number(v.balance).toLocaleString('fr-FR') }}
                                    </td>
                                    <td class="px-3 py-2 text-center">
                                      <span :class="['text-[10px] px-2 py-0.5 rounded-full border',
                                        v.statut === 'Acquitté'
                                          ? 'bg-[#1a4731] border-[#2ea043]/30 text-[#3fb950]'
                                          : v.statut.startsWith('Acqt')
                                          ? 'bg-[#1d2a3a] border-[#58a6ff]/30 text-[#58a6ff]'
                                          : v.statut.startsWith('Avance')
                                          ? 'bg-[#2d2a00] border-[#d29922]/30 text-[#d29922]'
                          ...
                          <!-- 🧾 Vie scolaire (depuis studentDetails.presences) -->
                          <div>
                            <p class="text-[11px] uppercase tracking-widest text-[#7d8590] mb-2 flex items-center gap-1">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                              <div v-for="stat in [
                                { label: 'Présences',
                                  value: (studentDetails?.presences ?? []).filter(pr => pr.valeur).length || '—',
                                  color: 'text-[#3fb950]' },
                                { label: 'Absences',
                                  value: (studentDetails?.presences ?? []).filter(pr => !pr.valeur).length || '—',
                                  color: 'text-[#f85149]' },
                                { label: 'Total jours',
                                  value: (studentDetails?.presences ?? []).length || '—',
                                  color: 'text-[#58a6ff]' },
                                { label: 'Taux présence',
                                  value: (studentDetails?.presences ?? []).length
                                    ? Math.round((studentDetails.presences.filter(pr => pr.valeur).length / studentDetails.presences.length) * 100) + '%'
                                    : '—',
                                  color: 'text-[#d29922]' },
                              ]" :key="stat.label"
                  ...
                  <div v-for="(p, i) in studentDetails.etudiant_facultes" :key="`fac-${i}`">
                    ...
                    <Transition name="accordion">
                      <div v-if="openAccordionIndex === 100+i" class="px-6 pb-4">
                        <p class="text-[12px] text-[#7d8590]">Session / Année d'étude : {{ p.classes?.nom_classe }}</p>
                      </div>
                    </Transition>
                  </div>
                </template>
