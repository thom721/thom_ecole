...
const canWrite = computed(() => authStore.hasPermission('Ajouter parametre') || authStore.isAdmin);
...
const visibleTabs = computed(() => {
  ...
  return ALL_TABS.filter(t => tabIds.includes('settings.' + t.id))
})

const activeTab = ref('exams');
watch(visibleTabs, (vt) => {
  if (vt.length && !vt.find(t => t.id === activeTab.value)) {
    activeTab.value = vt[0].id
  }
}, { immediate: true })
...
// ── Forms ──────────────────────────────────────────────────────
const form = ref({ id: '', nom: '', nb_annee: '', description: '', status: true, errors: {} });
...
const facModalClose = () => { faculte_profession.value = false; changeButton.value = false; form.value = { id: '', nom: '', nb_annee: '', description: '', status: true, errors: {} }; facImageFile.value = null; facImagePreview.value = null; };
const editFac = (fac) => {
  form.value = { id: fac.id, nom: fac.nom, nb_annee: fac.nb_annee, description: fac.description || '', status: !!fac.status, errors: {} };
  facImagePreview.value = fac.image_url ? (fac.image_url.startsWith('http') ? fac.image_url : `${url}${fac.image_url}`) : null;
  facImageFile.value = null;
  changeButton.value = true;
  facModalShow();
};
const facImageFile = ref(null);
const facImagePreview = ref(null);
const onFacImageChange = (e) => {
  const file = e.target.files[0];
  if (!file) return;
  facImageFile.value = file;
  const reader = new FileReader();
  reader.onload = (ev) => { facImagePreview.value = ev.target.result; };
...
const submitFac = async () => {
  try {
    ...
    if (res.status === 200) {
      if (facImageFile.value && res.data?.id) {
        const fd = new FormData();
        fd.append('file', facImageFile.value);
        await axios.post(`${url}/facultes/${res.data.id}/upload-image`, fd);
      }
      showSwalSuccess('Faculté ajoutée avec succès'); await getFacultes(); facModalClose();
    }
  } catch (e) {
...
<template>
  ...
  >
      ...
      >
        ...
        <div v-show="activeTab === 'exams'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              ...
              <button v-if="canWrite" @click="examParamsModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
              ...
              <table class="w-full text-[13px]">
                ...
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="exam in examPaginate.data" :key="exam.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td v-if="canWrite" class="px-6 py-3 text-right" :data-set="exam.id" @click="actionsOnClasse">
        ...
        <div v-show="activeTab === 'facultes'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              ...
              <button v-if="canWrite" @click="facModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path d="M8.75 3.75a.75.75 0 0 0-1.5 0v3.5h-3.5a.75.75 0 0 0 0 1.5h3.5v3.5a.75.75 0 0 0 1.5 0v-3.5h3.5a.75.75 0 0 0 0-1.5h-3.5v-3.5Z"/></svg>
                Ajouter
              </button>
            </div>
                ...
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="fac in facultePaginate.data" :key="fac.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-3 text-right">
                      ...
                      <button @click="editFac(fac)" class="inline-flex items-center justify-center w-7 h-7 rounded-lg hover:bg-amber-500/10 text-[#3d4d62] hover:text-amber-400 transition-colors" title="Modifier">
        ...
        <div v-show="activeTab === 'annees'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              ...
              <button v-if="canWrite" @click="yearModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path d="M8.75 3.75a.75.75 0 0 0-1.5 0v3.5h-3.5a.75.75 0 0 0 0 1.5h3.5v3.5a.75.75 0 0 0 1.5 0v-3.5h3.5a.75.75 0 0 0 0-1.5h-3.5v-3.5Z"/></svg>
                Ajouter
              </button>
            </div>
              ...
              <table class="w-full text-[13px]">
                ...
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="anne in annee_paginate.data" :key="anne.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td v-if="canWrite" class="px-6 py-3 text-right" :data-set="anne.id" @click="actionsOnClasse">
        ...
        <div v-show="activeTab === 'classes'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              <div>
                <h2 class="text-[14px] font-semibold text-[#e8eaf0]">Sessions / Années d'étude</h2>
                <p class="text-[12px] text-[#7c83a0] mt-0.5">Organisez les sessions/années d'étude par cycle</p>
              </div>
              <button v-if="canWrite" @click="niveauModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path d="M8.75 3.75a.75.75 0 0 0-1.5 0v3.5h-3.5a.75.75 0 0 0 0 1.5h3.5v3.5a.75.75 0 0 0 1.5 0v-3.5h3.5a.75.75 0 0 0 0-1.5h-3.5v-3.5Z"/></svg>
                Ajouter
              </button>
            </div>
              ...
              <table class="w-full text-[13px]">
                <thead>
                  <tr class="border-b border-white/[0.05] bg-[#0d1117]/60">
                    ...
                    <th class="px-6 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-[#3d4d62]">Session / Année d'étude</th>
                    <th class="px-6 py-3 text-right text-[10px] font-semibold uppercase tracking-wider text-[#3d4d62]">Actions</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="cls in classeParams.data" :key="cls.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td v-if="canWrite" class="px-6 py-3 text-right" :data-set="cls.id" @click="actionsOnClasse">
        ...
        <div v-show="activeTab === 'paiements'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              ...
              <button v-if="canWrite" @click="paymentParamsModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path d="M8.75 3.75a.75.75 0 0 0-1.5 0v3.5h-3.5a.75.75 0 0 0 0 1.5h3.5v3.5a.75.75 0 0 0 1.5 0v-3.5h3.5a.75.75 0 0 0 0-1.5h-3.5v-3.5Z"/></svg>
                Ajouter
              </button>
            </div>
              ...
              <table class="w-full text-[13px]">
                ...
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="pp in paimentParams.data" :key="pp.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td v-if="canWrite" class="px-6 py-3 text-right" :data-set="pp.id" @click="actionsOnClasse">
        ...
        <div v-show="activeTab === 'frais'">
          <div class="bg-[#161b26] rounded-2xl border border-white/[0.07] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
              ...
              <button v-if="canWrite" @click="fraisModalShow" class="inline-flex items-center gap-2 px-3.5 py-2 bg-[#4f8ef7]/15 text-[#7aaeff] border border-[#4f8ef7]/25 rounded-xl text-[12px] font-medium hover:bg-[#4f8ef7]/25 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path d="M8.75 3.75a.75.75 0 0 0-1.5 0v3.5h-3.5a.75.75 0 0 0 0 1.5h3.5v3.5a.75.75 0 0 0 1.5 0v-3.5h3.5a.75.75 0 0 0 0-1.5h-3.5v-3.5Z"/></svg>
                Ajouter
              </button>
            </div>
              ...
              <table class="w-full text-[13px]">
                ...
                <tbody class="divide-y divide-white/[0.04]">
                  ...
                  <tr v-else v-for="f in frais_paginate" :key="f.id" class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td v-if="canWrite" class="px-6 py-3 text-right">
                      <button class="inline-flex items-center justify-center w-7 h-7 rounded-lg hover:bg-red-500/10 text-[#3d4d62] hover:text-red-400 transition-colors mr-1">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-3.5 h-3.5"><path fill-rule="evenodd" d="M5 3.25V4H2.75a.75.75 0 0 0 0 1.5h.3l.815 8.15A1.5 1.5 0 0 0 5.357 15h5.285a1.5 1.5 0 0 0 1.493-1.35l.815-8.15h.3a.75.75 0 0 0 0-1.5H11v-.75A2.25 2.25 0 0 0 8.75 1h-1.5A2.25 2.25 0 0 0 5 3.25Z" clip-rule="evenodd"/></svg>
                      </button>
    ...
    <StyleModal :show="faculte_profession" @close="facModalClose">
      ...
      <template #content>
        <form @submit.prevent="submitFac" class="space-y-4">
          ...
          <div>
            <InputLabel for="fac_description" value="Description (facultatif)" class="text-[#7c83a0]" />
            <textarea id="fac_description" v-model="form.description" rows="3"
              class="mt-1 block w-full rounded-md bg-[#0d1117] border-white/[0.1] text-[#e8eaf0] text-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
            <InputError class="mt-1" :message="form.errors.description" />
          </div>
          <div>
            <InputLabel for="fac_image" value="Image (facultatif)" class="text-[#7c83a0]" />
            <img v-if="facImagePreview" :src="facImagePreview" alt="Aperçu" class="mt-1 mb-2 w-full h-32 object-cover rounded-lg border border-white/[0.1]" />
            <input id="fac_image" type="file" accept="image/*" @change="onFacImageChange"
              class="mt-1 block w-full text-xs text-[#7c83a0] file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-500/10 file:text-indigo-400 hover:file:bg-indigo-500/20" />
          </div>
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" v-model="form.status" class="rounded border-white/[0.2] bg-[#0d1117] text-indigo-500 focus:ring-indigo-500" />
            <span class="text-[13px] text-[#c9d1d9]">Faculté active (visible publiquement)</span>
          </label>
    ...
    <StyleModal :show="niveau_detude_modal_classe" @close="niveauModalClose">
      <template #title>
        <h5 class="text-[#e8eaf0] font-semibold">Session / Année d'étude</h5>
      ...
      <template #content>
        <form @submit.prevent="submitNiveau" class="space-y-4">
          ...
          <div>
            <InputLabel for="nom_classe" value="Session / Année d'étude" class="text-[#7c83a0]" />
    ...
    <StyleModal :show="Payment_params_show" @close="paymentParamsModalClose">
      ...
      <template #content>
        ...
        <form @submit.prevent="submitPaymentParams" class="space-y-4">
          ...
          <div>
            <InputLabel for="classe" value="Session / Année d'étude" class="text-[#7c83a0]" />
            <select class="select mt-1 w-full bg-[#0d1117] border-white/[0.1] text-[#e8eaf0]" id="classe" v-model="paymentFormParams.classe">
              <option value="">Choisir une session/année d'étude</option>
