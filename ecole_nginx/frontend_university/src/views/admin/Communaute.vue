<template>
  <div class="animate-[fadeUp_0.3s_ease_both]">
    ...
    <div v-if="activeSub === 'actualite'">
      ...
      <div v-else class="space-y-3">
        <div v-for="a in actualitesFiltrees" :key="a.id"
          ...
          <p class="text-xs text-[#7c83a0] leading-relaxed line-clamp-3">{{ stripHtml(a.content) }}</p>
        </div>
      </div>
    </div>

    <!-- ── Vidéos ── -->
    <div v-if="activeSub === 'video'">
      <div class="flex justify-end mb-4">
        <button @click="openVideoModal()" class="text-xs font-medium text-white px-3 py-1.5 rounded-lg hover:opacity-80 transition-opacity shrink-0" :style="{ background: 'var(--accent)' }">
          + Nouvelle vidéo
        </button>
      </div>

      <div v-if="loadingVideos" class="space-y-3">
        <div v-for="i in 3" :key="i" class="bg-[#171b26] border border-white/[0.07] rounded-xl p-5 animate-pulse">
          <div class="h-3 w-32 bg-white/[0.06] rounded mb-3"></div>
          <div class="h-4 w-2/3 bg-white/[0.06] rounded"></div>
        </div>
      </div>

      <div v-else-if="videos.length === 0" class="text-center py-16 text-[#7c83a0] text-[13px]">
      ...
      <div v-else class="space-y-3">
        <div v-for="v in videos" :key="v.id"
          ...
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="w-1.5 h-1.5 rounded-full shrink-0" :style="{ background: 'var(--accent)' }"></span>
              <span class="font-mono text-xs text-[#7c83a0]">{{ formatDate(v.created_at) }}</span>
              <span v-if="v.source" class="inline-flex items-center px-2 py-0.5 rounded-md text-[11.5px] font-medium bg-white/[0.06] text-[#a0a8c0]">{{ v.source }}</span>
              <span :class="['text-[10px] px-1.5 py-0.5 rounded font-medium', v.is_published ? 'bg-emerald-500/15 text-emerald-400' : 'bg-white/[0.06] text-[#7c83a0]']">
                {{ v.is_published ? 'Publié' : 'Brouillon' }}
              </span>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <button @click="openVideoModal(v)" class="text-[#7c83a0] hover:text-[var(--accent)] transition-colors">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/>
                </svg>
              </button>
              <button @click="deleteVideo(v.id)" class="text-[#7c83a0] hover:text-red-400 transition-colors">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/>
                </svg>
              </button>
          ...
          <h3 class="text-[13.5px] font-semibold text-[#e8eaf0] mb-1.5 leading-snug">{{ v.titre }}</h3>
          <p class="text-xs text-[#7c83a0] leading-relaxed truncate">{{ v.youtube_url }}</p>
        </div>
      </div>
    </div>
        ...
        <div class="bg-[#0f1117] border border-white/[0.1] rounded-2xl w-full max-w-lg shadow-2xl">
          ...
          <div class="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <QuillEditor
                v-model:content="newsForm.content" content-type="html"
                toolbar="essential" theme="snow" class="news-editor"
              />
            </div>
            <!-- Image -->
            <div>
    ...
    <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100"
                leave-active-class="transition duration-150" leave-to-class="opacity-0">
      <div v-if="showVideoModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
           @click.self="showVideoModal = false">
        <div class="bg-[#0f1117] border border-white/[0.1] rounded-2xl w-full max-w-lg shadow-2xl">
          <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.07]">
            <h2 class="text-[14px] font-semibold text-[#e8eaf0]">{{ editingVideo ? 'Modifier la vidéo' : 'Nouvelle vidéo' }}</h2>
            <button @click="showVideoModal = false" class="text-[#7c83a0] hover:text-[#e8eaf0] transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <div class="px-6 py-5 space-y-4">
            <div>
              <label class="block text-[11px] font-medium text-[#7c83a0] uppercase tracking-wider mb-1.5">Titre *</label>
              <input v-model="videoForm.titre" type="text" placeholder="Titre de la vidéo"
                class="w-full bg-[#171b26] border border-white/[0.1] rounded-lg px-3 py-2 text-[13px] text-[#e8eaf0] focus:outline-none focus:border-[var(--accent)]/50 transition"/>
            </div>
            <div>
              <label class="block text-[11px] font-medium text-[#7c83a0] uppercase tracking-wider mb-1.5">Lien YouTube *</label>
              <input v-model="videoForm.youtube_url" type="text" placeholder="https://www.youtube.com/watch?v=..."
                class="w-full bg-[#171b26] border border-white/[0.1] rounded-lg px-3 py-2 text-[13px] text-[#e8eaf0] focus:outline-none focus:border-[var(--accent)]/50 transition"/>
            </div>
            <div>
              <label class="block text-[11px] font-medium text-[#7c83a0] uppercase tracking-wider mb-1.5">Source</label>
              <input v-model="videoForm.source" type="text" placeholder="Ex: RTCH NEWS"
                class="w-full bg-[#171b26] border border-white/[0.1] rounded-lg px-3 py-2 text-[13px] text-[#e8eaf0] focus:outline-none focus:border-[var(--accent)]/50 transition"/>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" v-model="videoForm.is_published" class="w-4 h-4 accent-[var(--accent)]"/>
              <span class="text-[13px] text-[#e8eaf0]">Publier</span>
            </label>
          </div>
          <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/[0.07]">
            <button @click="showVideoModal = false" class="px-4 py-2 text-[13px] text-[#7c83a0] hover:text-[#e8eaf0] transition-colors">Annuler</button>
            <button @click="saveVideo" :disabled="savingVideo"
              class="px-4 py-2 text-[13px] font-medium text-white rounded-lg disabled:opacity-50 hover:opacity-90 transition-opacity"
              :style="{ background: 'var(--accent)' }">
              {{ savingVideo ? 'Enregistrement...' : (editingVideo ? 'Modifier' : 'Créer') }}
            </button>
          </div>
        </div>
      </div>
...
const videoForm = ref({ titre: '', source: '', youtube_url: '', is_published: true })
...
const ALL_COMMUNITY_TABS = [
  ...
  { key: 'video',     subId: 'communaute.videos',     label: 'Vidéos' },
...
const tabs = computed(() => {
  ...
  return base.map(t => ({
    ...t,
    count: t.key === 'evenement' ? evenements.value.length : t.key === 'actualite' ? actualites.value.length : t.key === 'video' ? videos.value.length : 0
  }))
})
watch(tabs, (vt) => {
...
// newsForm.content est du HTML (édité via QuillEditor) — cet aperçu compact
// ne doit afficher que le texte, pas les balises brutes.
...
const fetchVideos = async () => {
  ...
  try {
    ...
    videos.value = data
  } catch (e) { console.error('[Communaute] videos:', e) }
...
// ── CRUD Vidéos ────────────────────────────────────────────────────────────
const openVideoModal = (v = null) => {
  editingVideo.value = v
  videoForm.value = v
    ? { titre: v.titre, source: v.source ?? '', youtube_url: v.youtube_url, is_published: v.is_published }
    : { titre: '', source: '', youtube_url: '', is_published: true }
  showVideoModal.value = true
}

const saveVideo = async () => {
  if (!videoForm.value.titre) { Swal.fire({ icon: 'warning', title: 'Titre requis', background: '#0f1117', color: '#e8eaf0' }); return }
  if (!videoForm.value.youtube_url) { Swal.fire({ icon: 'warning', title: 'Lien YouTube requis', background: '#0f1117', color: '#e8eaf0' }); return }
  savingVideo.value = true
  try {
    if (editingVideo.value) {
      await axios.put(`${url}/videos/${editingVideo.value.id}`, videoForm.value)
    } else {
      await axios.post(`${url}/videos/`, videoForm.value)
    }
    ...
    const detail = e.response?.data?.detail
    Swal.fire({ icon: 'error', title: 'Erreur', text: Array.isArray(detail) ? detail.map(d => d.msg).join(', ') : (detail || 'Erreur inconnue'), background: '#0f1117', color: '#e8eaf0' })
  } finally { savingVideo.value = false }
}

const deleteVideo = async (id) => {
  const res = await Swal.fire({ title: 'Supprimer cette vidéo ?', icon: 'warning', showCancelButton: true,
    confirmButtonColor: '#ef4444', cancelButtonColor: '#374151', confirmButtonText: 'Supprimer', cancelButtonText: 'Annuler',
    background: '#0f1117', color: '#e8eaf0' })
  if (!res.isConfirmed) return
  try { await axios.delete(`${url}/videos/${id}`); await fetchVideos() }
  catch (e) { Swal.fire({ icon: 'error', title: 'Erreur', text: e.response?.data?.detail || 'Erreur', background: '#0f1117', color: '#e8eaf0' }) }
}

onMounted(() => { fetchEvents(); fetchNews(); fetchVideos() })
</script>
...
.news-editor :deep(.ql-toolbar) {
  background: #171b26; border-color: rgba(255,255,255,.1); border-radius: 8px 8px 0 0;
}
.news-editor :deep(.ql-toolbar .ql-stroke) { stroke: #7c83a0; }
.news-editor :deep(.ql-toolbar .ql-fill) { fill: #7c83a0; }
.news-editor :deep(.ql-toolbar .ql-picker-label) { color: #7c83a0; }
.news-editor :deep(.ql-container) {
  background: #171b26; border-color: rgba(255,255,255,.1); border-radius: 0 0 8px 8px;
  color: #e8eaf0; font-size: 13px; min-height: 140px;
}
.news-editor :deep(.ql-editor.ql-blank::before) { color: #7c83a0; font-style: normal; }
</style>
