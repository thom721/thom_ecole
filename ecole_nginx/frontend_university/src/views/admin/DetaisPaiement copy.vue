...
const check_echeance = computed(() =>
  paiement_details.value?.check_echeance ?? {}
);
...
const mois = computed(() =>
  DetailsData.value?.paiement_details?.mois ?? {}
);
...
<template>
  <div
    class="max-w-4xl mx-auto pb-8 px-4 sm:px-6 min-h-screen"
    ...
    <div v-if="Object.keys(info_paiement).length" class="space-y-4">
      ...
      >
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-3 border-t border-white/[0.05] bg-[#0d1117]/50">
          ...
          <!-- Imprimer reçu -->
          <button
            @click="printRecu(DetailsData.value.id, dateKey)"
