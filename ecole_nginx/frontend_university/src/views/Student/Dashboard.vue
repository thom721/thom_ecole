...
const statsCards = ref([
  { label: 'Moyenne Générale', value: '-', unit: '/10', icon: '📊', color: 'emerald', trend: '' },
  { label: 'Absences',         value: '-',    unit: 'jours', icon: '📅', color: 'amber',   trend: '' },
  { label: 'Cours validés',    value: '-',    unit: '/11',   icon: '✅', color: 'sky',     trend: '' },
  { label: "Rang de session/année d'étude", value: '-',    unit: 'ème',   icon: '🏅', color: 'violet',  trend: '' },
])
...
onMounted(async () => {
  ...
  if (dernierClasseEtudiant?.classes?.id) {
    reload_data_programme(dernierClasseEtudiant.annee_academiques.id, dernierClasseEtudiant.classes.id)
  }
...
const JOUR_ACTUEL = ['dimanche','lundi','mardi','mercredi','jeudi','vendredi','samedi'][new Date().getDay()]

function statutSeance(heure) {
  const now = new Date()
  const nowMin = now.getHours() * 60 + now.getMinutes()
  const [debut, fin] = (heure || '').split('-').map(h => h?.trim())
  const toMin = (h) => {
    const [hh, mm] = (h || '').split(':').map(Number)
    return (hh || 0) * 60 + (mm || 0)
  }
  if (!debut) return 'à venir'
  if (nowMin < toMin(debut)) return 'à venir'
  if (fin && nowMin > toMin(fin)) return 'terminé'
  return 'en cours'
}

const reload_data_programme = async (annee_academique_id, class_id) => {
  try {
    const response = await axios.post('/programme-and-student-cours', { annee_academique_id, class_id })
    coursAujourdhui.value = (response.data || [])
      .filter(c => JOURS_MAP[c.jours] === JOUR_ACTUEL)
      .map(c => ({
        heure:  c.heure?.split('-')[0]?.trim() || '',
        matiere: c.cours,
        salle:  '—',
        prof:   `${c.prof_nom || ''} ${c.prenom || ''}`.trim(),
        statut: statutSeance(c.heure),
      }))
      .sort((a, b) => a.heure.localeCompare(b.heure))
  } catch (error) {
    console.error('Erreur de chargement du programme:', error.response?.data || error.message)
  }
}
...
const reload_data = async (e,last_year)=>  {
  ...
  { label: "Rang de session/année d'étude", value: response.data?.result.rang,    unit: 'ème',   icon: '🏅', color: 'violet',  trend: `sur ${response.data?.student_value} élèves` },
]
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <div class="space-y-2.5">
              ...
              <p v-if="!coursAujourdhui.length" class="text-center text-sm text-slate-400 py-4">Aucun cours aujourd'hui</p>
            </div>
          </div>
        </div>
      ...
      <template v-if="activeTab === 'emploi'">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div class="divide-y divide-slate-50">
            ...
            <p v-if="!coursAujourdhui.length" class="text-center text-sm text-slate-400 py-8">Aucun cours aujourd'hui</p>
          </div>
        </div>
      </template>
