    ...
    <div class="mx-auto px-4 -mt-8 pb-16">
      ...
      <div class="absolute inset-0 opacity-10">
        ...
        <div class="flex-1 py-8">
          <h1 class="font-serif text-2xl font-bold text-white">{{ paiement?.details_etudiant?.nom || '—' }}</h1>
...
const paiement = computed(() => paiementData.value || {})
...
onMounted(async ()=>{
  ...
  try {
    ...
    const last_year = dernierClasseEtudiant?.annee_academiques?.annee_academique
    if (last_year) await reload_data_paie(last_year)
  } catch (error) {
    console.error('Erreur de chargement du profil étudiant:', error.response?.data || error.message)
  }
})
</script>
