...
// ── Moyenne générale (depuis result)
...
const currentBulletin = computed(() => {
  ...
  // const m = months.find(x => x.key === currentKey.value)
  return {
    titre:'Bulletin', //`Bulletin — ${m?.label || currentKey.value}`,
...
const currentStats = computed(() => [
  { label: 'Moyenne générale', value: moyenne.value, sub: 'sur 20 points',        color: '#c9a84c' },
  { label: 'Meilleure note',   value: bestNote.value?.note,  sub: bestNote.value?.nom,     color: '#1a7a4a' },
  { label: 'À améliorer',      value: weakNote.value?.note,  sub: weakNote.value?.nom,     color: '#c0392b' },
