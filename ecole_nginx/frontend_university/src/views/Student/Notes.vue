  ...
  <div class="min-h-screen bg-[#faf8f3] font-sans animate-[fadeUp_0.4s_ease_both]">
    ...
    <div class="mx-auto px-4 pb-16 -mt-8">
      ...
      <div class="flex gap-3 mb-4 items-center flex-wrap print:hidden">
        ...
        <select v-model="selectedMatiere" class="px-3 py-2 rounded-lg border-[1.5px] border-[#e0dbd0] bg-white text-sm text-[#0d0d14] outline-none focus:border-[#c9a84c] cursor-pointer">
          <option value="">Toutes les matières</option>
          <option v-for="m in matieresListe" :key="m" :value="m">{{ m }}</option>
        </select>
      </div>
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-[#0d0d14]">
              ...
              <th class="px-5 py-3.5 text-left text-[0.7rem] font-semibold tracking-widest uppercase text-[#888] hidden md:table-cell">Moy. Session/Classe</th>
...
const matieresListe = computed(() => Object.keys(dataEtudiant.value))

const notes = computed(() => {
  const entries = Object.entries(dataEtudiant.value)
    .filter(([matiere]) => !selectedMatiere.value || matiere === selectedMatiere.value)
...
const currentStats = computed(() => [
  ...
  { label: "Rang / Session-Année d'étude", value: rang.value,         sub: `sur ${student_value.value} élèves`,  color: '#1a3a6b', sup: 'e' },
])
...
const badges = computed(() => rang.value === 1 ? [{ label: '🏅 Major de classe', type: 'gold' }] : [])
