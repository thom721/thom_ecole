...
<template>
  <div class="min-h-screen bg-[#0f1117] text-[#e8eaf0] font-sans flex" :style="cssVars">
      ...
      >
        ...
        >
          ...
          >
            <div v-if="!sidebarCollapsed || isMobile" class="overflow-hidden whitespace-nowrap">
              <div class="text-[15px] font-semibold text-[#e8eaf0] leading-tight" style="font-family:'Playfair Display',serif">{{  useSchoolInfo.school_info?.nom }}</div>
        ...
        >
          ...
          <div v-if="!sidebarCollapsed || isMobile" class="overflow-hidden">
            <p class="text-[13px] font-medium text-[#e8eaf0] truncate">{{ teacherName }}</p>
            <p class="text-[11px] text-[#7c83a0] truncate">{{ teacherRole }}</p>
          </div>
        </div>
    ...
    <div
      ...
      :class="{
        'ml-[240px]': sidebarOpen && !isMobile && !sidebarCollapsed,
        'ml-[68px]':  sidebarOpen && !isMobile && sidebarCollapsed,
        'ml-0':       !sidebarOpen || isMobile,
      }"
    >
      <!-- TOPBAR -->
      <header class="sticky top-0 z-10 flex items-center gap-3 px-5 h-14 bg-[#0f1117]/90 backdrop-blur-md border-b border-white/[0.07] flex-shrink-0">
        ...
        <div class="flex items-center gap-2 text-[13px]">
          <span class="text-[#7c83a0]">{{  useSchoolInfo.school_info?.nom }}</span>
          <span class="text-[#7c83a0]/40">/</span>
          <span class="text-[#e8eaf0] font-medium">{{ currentRouteLabel }}</span>
        </div>
        ...
        <div class="flex items-center gap-1">
          ...
          <button class="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-white/[0.06] transition-colors text-[#7c83a0] hover:text-[#e8eaf0]">
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
          ...
          <button @click="toggleProfileDropdown"
            class="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-gray-900 transition-colors cursor-pointer">
            <!-- Avatar -->
            <div class="w-8 h-8 rounded-full overflow-hidden shrink-0">
              <img v-if="authStore.user?.profile_photo_url" :src="user.profile_photo_url" :alt="authStore.user?.user?.username" class="w-full h-full object-cover" />
              <div v-else class="w-full h-full bg-gray-700 flex items-center justify-center text-white text-xs font-bold">
                {{ authStore.user?.user?.username?.charAt(0)?.toUpperCase() ?? '?' }}
              </div>
            </div>
            <!-- Nom -->
            <span class="text-sm font-medium text-slate-700 hidden md:block max-w-[120px] truncate">
              {{ authStore.user?.user?.username ?? 'Utilisateur' }}
            </span>
            <!-- Chevron -->
          ...
          <Transition
            ...
            <div v-if="profileDropdown"
              class="absolute right-0 top-full mt-1.5 w-52 bg-gray-800 rounded-xl border border-slate-900 shadow-lg overflow-hidden z-50">

              <!-- Entête -->
              <div class="px-4 py-3 border-b border-slate-700 bg-[#171b26]">
                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Mon compte</p>
                <p class="text-sm font-semibold text-slate-700 mt-0.5 truncate">{{ authStore.user?.user?.username ?? 'Utilisateur' }}</p>
                <p class="text-xs text-slate-400 truncate">{{ authStore.user?.user?.email }}</p>
              </div>

              <div class="py-1">
                <RouterLink :to="{ name: 'teacher.profile' }" @click="closeProfileDropdown"
                  class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-600 hover:bg-white/[0.07] hover:text-slate-400 transition-colors">
      ...
      <!-- FOOTER -->
      <footer class="border-t border-white/[0.07] px-8 py-3 flex items-center justify-center text-[11px] text-[#7c83a0] flex-shrink-0">
        <span>{{ useSchoolInfo.school_info?.nom|| 'Lekol360' }} © {{ new Date().getFullYear() }}</span>
      </footer>
...
const props = defineProps({
  ...
  appSub:          { type: String, default: 'Espace Enseignant' },
  appInitial:      { type: String, default: 'E' },
  teacherName:     { type: String, default: 'Marie Soleil' },
  teacherInitials: { type: String, default: 'MS' },
  teacherRole:     { type: String, default: 'Mathématiques · Terminale' },
    ...
    default: () => [
      {
        label: 'Navigation',
        items: [
          { name: 'teacher.dashboard',  label: 'Tableau de Bord',   to: '/professeur-dashboard',              badge: null,
          ...
          { name: 'teacher.classes',    label: 'Mes Classes',        to: 'professeur-classes',       badge: '6',
          ...
          { name: 'teacher.notes',      label: 'Devoirs & Notes',    to: 'professeur-notes',         badge: null,
          ...
          { name: 'teacher.cours',      label: 'Cours & Ressources', to: 'professeur-cours',         badge: null,
          ...
          { name: 'teacher.messages',   label: 'Messages',           to: 'professeur-messages',      badge: '3',
            icon: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" width="18" height="18"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>` },
        ]
      },
      {
        label: 'Gestion',
        items: [
          { name: 'teacher.calendrier',   label: 'Calendrier',   to: 'professeur-calendrier',   badge: null,
          ...
          { name: 'teacher.statistiques', label: 'Statistiques', to: 'professeur-statistiques', badge: null,
          ...
          { name: 'teacher.parametres',   label: 'Profile',   to: 'professeur-parametres',   badge: null,
            icon: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" width="18" height="18"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93A10 10 0 104.93 19.07 10 10 0 0019.07 4.93z"/></svg>` },
        ]
      } //Paramètres
    ]
  }
...
const currentRouteLabel = computed(() => {
  for (const s of props.navSections) {
    const item = s.items.find(i => i.to === route.path)
...
const toggleSidebar = () => {
  ...
  else { sidebarOpen.value = !sidebarOpen.value; if (!sidebarOpen.value) sidebarCollapsed.value = false }
}
const toggleCollapse = () => { if (!isMobile.value) sidebarCollapsed.value = !sidebarCollapsed.value }
...
onMounted( async () => { 
  checkMobile(); window.addEventListener('resize', checkMobile); document.addEventListener('click', onOutsideClick) ;  if (!authStore.user) await authStore.initializeAuth();
...
onUnmounted(() => { window.removeEventListener('resize', checkMobile); document.removeEventListener('click', onOutsideClick) })
