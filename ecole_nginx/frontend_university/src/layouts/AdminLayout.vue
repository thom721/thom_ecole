...
const shouldShowMenuItem = (itemName) => {
  ...
  switch (itemName) {
    ...
    case 'Crédits':
      ...
      // (voir frontend/AdminLayout.vue, même logique) — chaque onglet gère
      // sa propre visibilité de contrôles à l'intérieur de Credits.vue.
...
onMounted(async () => {
  ...
  // voir shouldShowMenuItem) ait des données — voir frontend/AdminLayout.vue.
          ...
          <div id="profile-menu" class="relative">
            ...
            >
              ...
              >
                ...
                <div class="py-1">
                  <!-- Compte admin qui porte aussi le rôle enseignant (casquette
                  Professeur liée sans compte de connexion propre — voir
                  _sync_shadow_professeur, RAcademic.py) : accès à l'espace prof
                  sans avoir à se reconnecter avec une identité "Professeur"
                  séparée, qui n'existe pas pour ce compte. -->
                  <RouterLink
                    v-if="authStore.roleNames.includes('teacher')"
                  ...
                  >
                    <svg class="w-4 h-4 text-[#7c83a0]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.42A12.083 12.083 0 0112 21a12.083 12.083 0 01-6.16-10.42L12 14z" />
                    </svg>
                    Espace Professeur
                  </RouterLink>
                  <RouterLink
