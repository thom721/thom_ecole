from PySide6.QtWidgets import (
    QDialog, QPushButton, QVBoxLayout, QComboBox, QLabel, QLineEdit, QFrame
)
...
from Helper.Modal_close import fade_out_and_close
class Frais_dinscritipn(QDialog):
    ligne_ajoutee = Signal()

    def __init__(self,frais_edit):
        super().__init__()
        # self = QDialog()
        self.setWindowTitle("Frais d'inscription")
    ...
    def enregistrer_frais_inscription(self, niveaux, AnneAcademique):
        ...
        label_frais = QLabel("Montant des frais :")
        ...
        if self.frais_edit:
            ...
            self.input_frais.setText(str(frais_to_edit.get("prix", "")))
            annee = frais_to_edit.get("anneeAc","")
            niveau = frais_to_edit.get("niveau_id","")
    ...
    def enregistrer(self):
        ...
        response = self.api_handler_.enregistrer_frais_dinscription(id=self.id_input_frais.text(),prix=montant, niveau_id=niveau, anneeAc=annee)
    ...
    def generic_success_handler(self, endpoint, method, response_data): 
        print(response_data)
        if endpoint.startswith("v1/fraisDinscription"):
            if response_data and 'success' in response_data:
                self.frais_edit=None
                self.ligne_ajoutee.emit()
                fade_out_and_close(self, 500)
                # self.close()
                # fade_out_and_close(self)
